	...
	validateToken()
		...
		.then(attachments => {
			attachments.forEach(attachment => {
				const pathToStore = path.join(__dirname, '../temp');
				const pathWithFileName = `${pathToStore}/${Math.random() * 100 + 1}.csv`;

				if (!fs.existsSync(pathToStore)) {
					fs.mkdirSync(pathToStore);
				}

				fs.promises.writeFile(pathWithFileName, attachment.data.data, 'base64').then(() => converter(pathWithFileName));
			});
		})
		.catch(error => {});
});
	...
	validateToken()
		.then(tokenAndoAuthClient => {
			const gmail = google.gmail({
				version: 'v1',
				auth: tokenAndoAuthClient.oAuth2Client,
			});
