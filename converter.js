...
const converter = (pathtofile, originalFileName = 'unNamedFile') => {
	...
	const dateToAddInFileName = `${date.getUTCDate()}-${date.getUTCMonth() + 1}-${date.getUTCFullYear()}__${date.getUTCHours()}-${date.getUTCMinutes()}-${date.getUTCSeconds()}`;
	...
	if (sheetToFormat['A1'].v === 'H') {
		...
		XLSX.writeFile(formattedWorkbook, `./public/output/valid__${dateToAddInFileName}__${randomFileName}__${invoice.number}.csv`);

		fs.promises.unlink(pathtofile).catch(error => {});
	} else {
		fs.copyFile(pathtofile, `./public/output/invalid__${dateToAddInFileName}__${originalFileName}`, error => {});
	}
};
